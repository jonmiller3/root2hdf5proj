        ...
        bool norm_to_max)
{
    ...
    for (i = first_event_number; i < entries_mc; ++i) {
        ...
        if (open_new_file) {
            ...
            skim_file << "# Entry interp: view:energy " << 
            ...
            skim_file << "# Segment Z plane-id-code Run Subrun Gate/NthMCEvt Slices[0] ";
            for (unsigned int j = 0; j < mc->getLatticeSize(); ++j) {
                int lattice_idx = mc->latticeEnergyIndices[j];
        ...
        // really, "segment"
        int true_target = -1e6;
        int plane_id_code = -1e6;
        if (!is_data) {
            true_target = utils.getSegmentForZPosition(true_z);
            plane_id_code = utils.getPlaneIdCode(mc);
        }

        // get the max energy so we can scale 0->1
        double max_e = 0.0;
        double norm_factor = 0.0;
        if (norm_to_max) {
            for (unsigned int j = 0; j < mc->getLatticeSize(); ++j) {
                double energy_at_idx = mc->latticeNormEnergySums[j];
                if (energy_at_idx > max_e) {
                    max_e = energy_at_idx;
                }
            }
            norm_factor = max_e;
        ...
        // if max e is gt 0, we have an event 
        // loop through all indices and record them
        if (norm_factor > 0.0) {
            std::stringstream ss;
            ss << true_target << " " << true_z << " " << plane_id_code;
            if (is_data) {
            ...
            for (unsigned int j = 0; j < mc->getLatticeSize(); ++j) {
                ...
                std::stringstream ss;
                ss << " " << view << ":" << (energy_at_idx / norm_factor);
                skim_file << ss.str();
            }
            skim_file << std::endl;
        }
